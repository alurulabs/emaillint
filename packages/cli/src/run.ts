...
import type { RuleSetting, ClientId, ProfileName } from "emaillint-core";
...
export async function run(
  paths: string[],
  opts: { rules?: Record<string, RuleSetting>; clients?: ClientId[]; profile?: ProfileName; collectCtx?: boolean },
): Promise<RunResult> {
  ...
  for (const path of sorted) {
    try {
      ...
      const result = analyze(html, { rules: opts.rules, clients: opts.clients, profile: opts.profile });
      results.push(
        opts.collectCtx
