...
```ts
import {
  analyze, getRule, getCompatDataVersion,
  PROFILES,
  createBaseline, diffAgainstBaseline, parseBaseline,
} from "emaillint-core";
...
// Severity profile - a tier shift over calibrated defaults (explicit rules still win):
analyze(html, { profile: "strict" });   // warnings -> errors
// PROFILES = { recommended: {}, strict: { warning: "error" }, relaxed: { warning: "info" } }

// Baseline mode - snapshot known errors, diff to fail only on new ones:
const baseline = createBaseline(files);                    // snapshot
const { newErrors, suppressed } = diffAgainstBaseline(     // count-based diff
  currentFiles, parseBaseline(json),
);
...
Client targeting uses generated caniemail IDs (the `ClientId` union, `CLIENTS`,
`CLIENT_IDS`, `CLIENT_PRESETS`): `analyze(html, { clients: ["gmail-desktop-webmail"] })`
drops compat issues fully supported across the selected clients.
...
`json`, and `sarif` (2.1.0). Flags include `--profile` (severity policy),
`--baseline` (tolerate existing debt), and `--preset` / `--clients` (target
clients). See [`@emaillint/cli`](https://www.npmjs.com/package/@emaillint/cli).
The package name `emaillint` is squatted on npm, so the CLI ships as the scoped
**`@emaillint/cli`** with bin **`emaillint`**.
